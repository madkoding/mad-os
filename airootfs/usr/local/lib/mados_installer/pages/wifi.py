...
from ..config import (
    DEMO_MODE,
    NORD_POLAR_NIGHT, NORD_SNOW_STORM, NORD_FROST, NORD_AURORA
)
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            ...
            if not line_stripped or '---' in line_stripped or 'Available' in line_stripped:
                continue
            # Parse network lines - iwd outputs: SSID   Security   Signal
            # Signal is shown as stars (****)
            parts = line_stripped.split()
            if len(parts) >= 2:
                # Find signal strength (the **** pattern)
                signal = ''
                security = 'Open'
                ssid_parts = []
                for part in parts:
                    if all(c == '*' for c in part) and len(part) <= 4 and part:
                        signal = part
                    elif part in ('open', 'wep', 'psk', 'wpa2-personal',
                                  'wpa1-personal', 'wpa2-enterprise',
                                  'wpa1-enterprise', '8021x'):
                        security = part.upper().replace('PSK', 'WPA2').replace(
                            'WPA2-PERSONAL', 'WPA2').replace('WPA1-PERSONAL', 'WPA1')
                    else:
                        ssid_parts.append(part)

                ssid = ' '.join(ssid_parts).strip()
                if ssid and ssid != 'Network' and ssid != 'name':
                    networks.append({
                        'ssid': ssid,
                        'signal': signal or '?',
                        'security': security
...
def create_wifi_page(app):
    ...
    header = create_page_header(app, app.t('wifi_setup'), 1, total_steps=9)
                ...
                if is_open:
                    ...
                    result = subprocess.run(
                        ['iwctl', '--passphrase', password,
                         'station', app.wifi_interface, 'connect', ssid],
                        capture_output=True, text=True, timeout=30
                    )
